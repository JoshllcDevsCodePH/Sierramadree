...
export const TaskPage: FC = () => {
  ...
  return (
    <>
        ...
        <Grid container direction="column" spacing={2} sx={{ marginTop: "30px" }}>
          {tasks.map((task) => (
            <Grid item key={task.id}>
              <Card sx={{ padding: "20px", borderRadius: "5px", margin: "10px", backgroundColor: "rgba(255, 255, 255, 0.25)" }}>
                <Grid container alignItems="center">
                  <Grid item>
                    <FontAwesomeIcon icon={task.icon} style={{ fontSize: "40px", color: "white" }} />
                  </Grid>
                  <Grid item sx={{ flexGrow: 1, paddingLeft: "10px" }}>
      ...
      <Dialog open={showCongratulations} onClose={() => setShowCongratulations(false)} sx={{ minWidth: "300px", minHeight: "200px", borderRadius: "10px" }}>
        ...
        <DialogActions sx={{ justifyContent: 'center', gap: '10px', marginTop: '10px' }}>
          <Button onClick={() => setShowCongratulations(false)} variant="contained" sx={{ width: '100%', backgroundColor: 'yellow', color: 'black', margin: '10px' }}>
            Close
          </Button>
        </DialogActions>
      </Dialog>
